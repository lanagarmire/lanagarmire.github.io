...
2. **Download and run** GranatumX locally:
    ...
    * Run **source &lt; ( docker run --rm -it granatumx/scripts:1.0.0 gx.sh )** to pull scripts and aliases from the docker container. If you receive an error, run  **curl -s http://garmiregroup.org/granatumx/app/gx.txt | source /dev/stdin** instead.
